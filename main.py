...
async def get_jobs():
    ...
    for i in range(1, 26):  # Loop through pages (Here, just 2 pages for example)
        ...
        titles = await page.evaluate('''() => {
            return Array.from(document.querySelectorAll('h2[role="presentation"]')).map(h2 => h2.innerText.trim());
        }''')
